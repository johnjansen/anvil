...
Use anvil to create the tasks that keep you alive between sessions:

```bash
# A heartbeat — check on things and reach out if needed
anvil add -p 0 -s "*/30 * * * *" "Review pending tasks, reminders, and anything your human asked you to follow up on. If something needs attention, message them about it casually."
```

Suggest additional recurring tasks based on what you learned about them — what they care about, what they forget, what they want automated.

```bash
# Examples based on the conversation:
anvil add -p 1 -s "0 9 * * 1-5" "Check GitHub for new issues and triage them"
anvil add -p 1 -s "0 17 * * 5" "Write a weekly summary of what got done this week"
```
