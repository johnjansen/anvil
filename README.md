...
When running in cluster mode, query cluster leadership status:

```bash
# Fetch cluster status from the daemon
curl --unix-socket ~/.anvil/daemon.sock http://daemon/cluster/status
```

Returns JSON with current leadership state, term, and peer information.
